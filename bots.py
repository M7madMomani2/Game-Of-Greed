...
class Mohammad(BaseBot):
    """TestBot banks the first roll always"""
    def _roll_bank_or_quit(self):
        if self.dice_remaining < 3 :
            return "b"


class Mohanad(BaseBot):
    """TestBot banks the first roll always risky"""
    def _roll_bank_or_quit(self):
...
if __name__ == "__main__":
    ...
    Mohammad.play(num_games)
    Tala.play(num_games)
    Rawan.play(num_games)
    Mohanad.play(num_games)
